...
Versão Final Simplificada (Leitura do GitHub)
Autor: Professor Dulluca
Data: 2026
Descrição: Dashboard Streamlit que lê dados diretamente do GitHub.
           Sem GCP, sem credenciais complexas, 100% gratuito.
"""
...
def carregar_dados_github():
    """
    Carrega os arquivos df_final.parquet e df_inpc.parquet diretamente do GitHub.
    
    IMPORTANTE: Substitua 'seu_usuario' e 'seu_repo' pelos seus valores reais.
    Exemplo: https://raw.githubusercontent.com/seu_usuario/pnad-dashboard/main/dados/df_final.parquet
    """
    try:
        # *** CONFIGURE AQUI OS SEUS DADOS DO GITHUB ***
        # Substitua 'seu_usuario' e 'seu_repo' pelos seus valores reais
        usuario_github = "seu_usuario"  # Seu usuário do GitHub
        repo_github = "seu_repo"         # Nome do seu repositório
        branch = "main"                  # Branch principal (geralmente 'main' ou 'master')
        
        # URLs dos arquivos no GitHub (raw content)
        ...
        st.error(f"""
        ❌ Erro ao carregar dados do GitHub: {e}
        
        **Verifique:**
        1. Os valores de `usuario_github` e `repo_github` estão corretos?
        2. Os arquivos `df_final.parquet` e `df_inpc.parquet` estão na pasta `dados/` do seu repositório?
        3. O repositório é público?
        4. A URL está correta? (verifique em https://github.com/{usuario_github}/{repo_github}/blob/{branch}/dados/)
        """)
        st.stop()
